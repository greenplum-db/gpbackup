...
var _ = Describe("restore/validate tests", func() {
	...
	Describe("GenerateRestoreRelationList", func() {
		BeforeEach(func() {
			...
			_ = cmdFlags.Set(utils.EXCLUDE_RELATION, "")
			_ = cmdFlags.Set(utils.INCLUDE_SCHEMA, "")
			_ = cmdFlags.Set(utils.EXCLUDE_SCHEMA, "")
		})
		...
		It("filters on include relations", func() {
			_ = cmdFlags.Set(utils.INCLUDE_RELATION, "s1.table1,s2.table2")
		...
		It("filters on exclude relations", func() {
			_ = cmdFlags.Set(utils.EXCLUDE_RELATION, "s1.table2,s2.table1")
			expectedRelations := []string{"s1.table1", "s2.table2"}
		...
		It("filters on include schema", func() {
			_ = cmdFlags.Set(utils.INCLUDE_SCHEMA, "s1")
		...
		It("filters on exclude schema", func() {
			_ = cmdFlags.Set(utils.EXCLUDE_SCHEMA, "s2")
			expectedRelations := []string{"s1.table1", "s1.table2"}
		...
		It("filters on include schema with exclude relation", func() {
			_ = cmdFlags.Set(utils.INCLUDE_SCHEMA, "s1")
			_ = cmdFlags.Set(utils.EXCLUDE_RELATION, "s1.table1")
	...
	Describe("ValidateRelationsInRestoreDatabase", func() {
		BeforeEach(func() {
			...
			_ = cmdFlags.Set(utils.DATA_ONLY, "false")
		})
		Context("data-only restore", func() {
			BeforeEach(func() {
				_ = cmdFlags.Set(utils.DATA_ONLY, "true")
			})
